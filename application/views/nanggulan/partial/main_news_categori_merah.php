...
if (!empty($data)) {
    foreach ($data as $b) {
        if (is_file("./" . $this->config->item('base_url_news_images') . "/normal/" . $b['gambar'])) {
			$img = '<img src="' . base_url($this->config->item('base_url_news_images') . "/normal/" . $b['gambar']) . '" alt="" class="img-responsive" style="height: 130px; width: 100%">';
		} else {
		...
		<div class="news-list-item articles-list">
			...
			<div class="post-info-2">
				<h4><a href="<?=base_url($url);?>" class="title"><?=htmlentities($b['judul']);?></a></h4>
				...
				<p class="hidden-sm" style="text-align: justify"><?=amankan_teks(substr(strip_tags($b['konten']), 0, 200));?>...</p>
			</div>
		</div>
        <?php
}
