...
class QuickMenu
{
    ...
    protected function build($array, $depth = 0)
    {
        ...
        foreach ($array as $item)
        {
            ...
            $str .= '<a href="' . base_url().$item['href'] . '" title="' . $item['title'] . '"' . $this->getAttr($a) . '>' . $this->getTextItem($item, $isParent) . '</a>';
            if ($isParent)
            {
    ...
    protected function buildFromResult(array $array, $parent = 0, $level = 0)
    {
        ...
        foreach ($array[$parent] as $item_id => $item)
        {
            ...
            $str .= "<a href=\"".base_url()."{$item['href']}\" target=\"{$item['target']}\"" . $this->getAttr($a) . '>' . $this->getTextItem($item, $isParent) . '</a>';
            if ($isParent)
            {
